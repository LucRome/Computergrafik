...
class MaterialTests(TestCase):
    def test_constructor(self):
        ...
        m = LightInformation(rgb, illu)
        self.assertTrue(np.equal(rgb, m.albedo).all())
    ...
    def test_interpolation(self):
        ...
        a = LightInformation(rgb1, illu1)
        b = LightInformation(rgb2, illu2)
        c = a.interpolate(b)
        self.assertTrue(np.equal(rgb_exp, c.albedo).all())
    ...
    def test_travel(self):
        ...
        [degrad, dis] = [0.1, 100]
        illu_exp = 90
        a = LightInformation([0,0,0], illu_start)
        b= a.travel(dis, degrad)
        self.assertEqual(illu_exp, b.illumination)
    
    def test_return(self):
        mat = LightInformation([240, 90, 4], 50)
        expected = [120, 45, 2]
